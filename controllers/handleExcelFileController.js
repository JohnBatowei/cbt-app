...
//         const subject = row.getCell(colIndex).value;
//         if (subject) {
//           subjectsRaw.push(subject.toString().trim());
//         }
//       }      
...
module.exports.handleExcelFileQuestion = [
  upload, // Multer middleware to handle multiple file uploads

  asyncHandler(async (req, res) => {
    const allQuestionsToInsert = [];
    const subjectMap = new Map(); // Cache to avoid duplicate DB queries
    ...
    if (allQuestionsToInsert.length > 0) {
      await questionModel.insertMany(allQuestionsToInsert);
      ...
      res.status(200).json({ message: `${allQuestionsToInsert.length} questions uploaded successfully.` });
    } else {
